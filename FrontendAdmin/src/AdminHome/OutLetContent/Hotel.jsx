...
export function Hotel() {
    ...
    const hotelData = useFetch('http://localhost:3100/admin/hotel');
    return (
        <>      <div className={style.list}>
                    <h1 className={style.h1}>Hotel List</h1>
                ...
                {hotelData && hotelData.map((data) => (
                  <tr key={data._id}>
                    ...
                    <button onClick={() => navigate(`/home/edit-hotel/${data._id}`)} className={style.buttonEdit}>Edit</button>
                    </td>
                  </tr>
                ))}
                <NavLink style={{position:"absolute", right:"50px", fontSize:"25px", height:"100px"}} to='/home/hotel-restore'><FontAwesomeIcon icon={faTrashCan}/></NavLink>
            </table>
        </>
    )
}